...
pub fn gam1(a: f64) -> f64 {
    ...
    if t > 0.0 {
        let top = ((((((P[6] * t + P[5]) * t + P[4]) * t + P[3]) * t + P[2]) * t + P[1]) * t)
            + P[0];
...
fn temme_general(a: f64, l: f64, z_in: f64, y: f64, rta: f64, _e: f64) -> (f64, f64) {
    ...
    const D0: [f64; 13] = [
        0.833333333333333e-1, -0.148148148148148e-1, 0.115740740740741e-2,
        0.352733686067019e-3, -0.178755144032922e-3, 0.391926317852244e-4,
        -0.218544851067999e-5, -0.185406221071516e-5, 0.829671134095309e-6,
        -0.176659527368261e-6, 0.670785354340150e-8, 0.102618097842403e-7,
        -0.438203601845335e-8,
    ];
    const D1: [f64; 12] = [
        -0.347222222222222e-2, 0.264550264550265e-2, -0.990226337448560e-3,
        0.205761316872428e-3, -0.401877572016461e-6, -0.180985503344900e-4,
        0.764916091608111e-5, -0.161209008945634e-5, 0.464712780280743e-8,
        0.137863344691572e-6, -0.575254560351770e-7, 0.119516285997781e-7,
    ];
    const D2: [f64; 10] = [
        -0.268132716049383e-2, 0.771604938271605e-3, 0.200938786008230e-5,
        -0.107366532263652e-3, 0.529234488291201e-4, -0.127606351886187e-4,
        0.342357873409614e-7, 0.137219573090629e-5, -0.629899213838006e-6,
        0.142806142060642e-6,
    ];
    const D3: [f64; 8] = [
        0.229472093621399e-3, -0.469189494395256e-3, 0.267720632062839e-3,
        -0.756180167188398e-4, -0.239650511386730e-6, 0.110826541153473e-4,
        -0.567495282699160e-5, 0.142309007324359e-5,
    ];
    const D4: [f64; 6] = [
        0.784039221720067e-3, -0.299072480303190e-3, -0.146384525788434e-5,
        0.664149821546512e-4, -0.396836504717943e-4, 0.113757269706784e-4,
    ];
    const D5: [f64; 4] = [
        -0.697281375836586e-4, 0.277275324495939e-3, -0.199325705161888e-3,
...
mod tests {
    ...
    fn psi_at_known_points() {
        ...
        assert!((psi(0.5) - expected).abs() < 1e-9, "psi(0.5) = {}", psi(0.5));
    }
    ...
    fn gamma_inc_at_a_half_uses_erf() {
        ...
        for &x in &[0.1, 0.5, 1.0, 4.0, 9.0] {
            ...
            assert!((p - expected).abs() < 1e-13, "x={x}: P = {p}, erf(√x) = {expected}");
        }
    }
}
