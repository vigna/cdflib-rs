//! Faithful Rust port of CDFLIB's `dzror` / `E0001` zero-finder.
//!
//! Algorithm R of Bus & Dekker (ACM TOMS 1975) — combined inverse
//! quadratic interpolation and bisection. The C version uses a
//! reverse-communication idiom driven by a `static`-local switch on a
//! "where to resume" integer `i99999`. The Rust port replaces that with
//! an explicit `Stage` enum carried inside [`ZrorState`].
//!
//! The exact iteration trace matches CDFLIB. Variable names follow the
//! C source (`a`, `b`, `c`, `d`, `fa`, `fb`, `fc`, `fd`, `fda`, `fdb`,
//! `m`, `mb`, `p`, `q`, `w`, `tol`, `ext`, `first`) for line-by-line
//! cross-referencing.
...
pub(crate) enum ZrorAction {
    ...
    /// `f(xlo)` and `f(xhi)` do not straddle zero. `qleft` / `qhi` follow
    /// the CDFLIB sign-flag convention.
    #[allow(dead_code)]
...
pub(crate) struct ZrorState {
    cfg: ZrorConfig,
    stage: Stage,
    // working state, names from the C source
    xlo: f64,
    xhi: f64,
    a: f64,
    b: f64,
