...
//! tolerances are configured to `1e-5` (chi²) and `1e-4` (F) inside the
//! C source. We assert at one digit tighter than each respective floor;
//! pushing for 1e-14 here would be testing against CDFLIB's own noise.

mod common;

use cdflib::{ChiSquaredNoncentral, ContinuousCdf, FisherSnedecorNoncentral};
use common::{
    assert_close_eps, read_csv, DEFAULT_ABS_TOL, NONCENTRAL_CHI_REL_TOL,
...
fn chi_squared_noncentral_matches_cumchn_reference() {
    for row in read_csv("tests/data/chi_squared_noncentral_cdf.csv") {
        let [df, ncp, x, expected_cdf, expected_sf] = row[..] else { panic!("width"); };
        let d = ChiSquaredNoncentral::new(df, ncp).unwrap();
        assert_close_eps(d.cdf(x), expected_cdf, NONCENTRAL_CHI_REL_TOL, DEFAULT_ABS_TOL);
        assert_close_eps(d.sf(x), expected_sf, NONCENTRAL_CHI_REL_TOL, DEFAULT_ABS_TOL);
    }
}
...
fn f_noncentral_matches_cumfnc_reference() {
    for row in read_csv("tests/data/fisher_snedecor_noncentral_cdf.csv") {
        let [dfn, dfd, ncp, fx, expected_cdf, expected_sf] = row[..] else { panic!("width"); };
        let d = FisherSnedecorNoncentral::new(dfn, dfd, ncp).unwrap();
        assert_close_eps(d.cdf(fx), expected_cdf, NONCENTRAL_F_REL_TOL, DEFAULT_ABS_TOL);
