...
//  Input:
//
//    double *A, *B, *X, the parameter values of the incomplete beta ratio.
//
...
//  Input:
//
//    double *A, *B, the parameter values of the function.
...
//  Output:
//
//    double beta_asym: the approximation.
//
//  Local:
//
//
...
//    function, and should satisy 0 <= X <= 1.  Y should equal 1 - X.
//
//    double *W, a quantity to which the
...
//  Output:
//
//    double *W: the result of the computation has been added 
//    to the input value.
//
//    int *IERR, an error flag, which is 0 if no error
//    was detected.
//
{
  double bm1;
...
//  Output:
//
//    double *W, *W1, the values of IX(A,B) and
//    1-IX(A,B).
//
...
//  Output:
//
//    double BETA_PSER, the approximate value of IX(A,B)(X).
//
{
  double a0;
  double apb;
  double bpser,b0,c,n,sum,t,tol,u,w,z;
  int i,m;
...
//
{
  ...
  double brcomp,b0,c,e,h,lambda,lnx,lny,t,u,v,x0,y0,z;
  int i,n;
//
//  CONST = 1/SQRT(2*PI)
//
...
//  Output:
//
//    double BETA_RCOMP1, the value of
...
//  Output:
//
//    double BETA_UP, the value of IX(A,B) - IX(A+N,B).
//
{
  double ap1;
  int K1 = 1;
...
//    output value of N_DATA will be 0 again.
//
//    int *A, double *B, the parameters of the function.
//
//    int *X, the argument of the function.
//
//    double *FX, the value of the function.
//
{
...
//
//    Computation of the other parameter values involves a seach for a value that
...
//  Input:
//
//    int *WHICH, indicates which of the next four argument
...
//
//    double *P, the integral from 0 to X of the
//    chi-square distribution.  Input range: [0, 1].
//
//    double *Q, equals 1-P.  Input range: [0, 1].
//
//    double *X, the upper limit of integration
//    of the beta density.  It should lie in
//    the range [0,1].
//
//    double *Y, equal to 1-X.  It should lie in the range [0,1].
//
//    double *A, the first parameter of the beta
//    density.  It should lie in the range
//    (0, +infinity).
//
//    double *B, the second parameter of the beta
//    density.  It should lie in the range
//    (0, +infinity).
//
//  Output:
...
//
//    double *X, the upper limit of integration
//    of the beta density.  It will be searched for
//    in the range [0,1].
//
//    double *Y, equal to 1-X.  It will be searched for in the range [0,1].
//
//    double *A, the first parameter of the beta
//    density.  It will be searched
...
//    for in the range [1D-300,1D300].
//
//    int *STATUS, reports the status of the computation.
...
//    +4, if X + Y /= 1.
//
//    double *BOUND, is only defined if STATUS is nonzero.
...
//  Input:
//
//    int *WHICH, indicates which of argument values is to
...
//
//    double *P, the cumulation, from 0 to S,
//    of the binomial distribution.  It should
//    lie in the range [0,1].
//
//    double *Q, equal to 1-P.  It should lie in the range [0,1].
//
//    double *S, the number of successes observed.
//    It should lie in the
//    range [0,XN].
//
//    double *XN, the number of binomial trials.
//    It should lie in the range: (0, +infinity).
//
//    double *PR, the probability of success in each
//    binomial trial.  It should
//    lie in the range: [0,1].
//
//    double *OMPR, equal to 1-PR.  It should lie in the range [0,1].  
//    Also, it should
...
//
//    double *Q, equal to 1-P.  It will lie in the range [0,1].
//
//    double *S, the number of successes observed.
//    It should lie in the
//    range [0,XN].
//
//    double *XN, the number of binomial trials.
//    It will be searched for in the
...
//  Input:
//
//    int *WHICH, indicates which argument is to be calculated
//    from the others.
//    1: Calculate P and Q from X and DF;
//    2: Calculate X from P, Q and DF;
//    3: Calculate DF from P, Q and X.
//
//    double *P, the integral from 0 to X of
//    the chi-square distribution.  It should
//    lie in the range [0,1].
//
//    double *Q, equal to 1-P.  It should lie in the range [0,1].
//
//    double *X, the upper limit of integration
//    of the chi-square distribution.  It should lie in the range: 
//    [0, +infinity). 
//
//    double *DF, the degrees of freedom of the
//    chi-square distribution.  It should lie
...
//
//    double *X, the upper limit of integration
//    of the chi-square distribution.  It will be searched for 
//    in the range: [0,1.0D+300].
//
//    double *DF, the degrees of freedom of the
//    chi-square distribution.  It will be
...
//
//    double *X, the upper limit of integration of the
...
//
//    double *Q, is generally not used by this
...
//
//    int *STATUS, reports on the calculation.
...
//
//    double *P, the integral from 0 to F of
//    the F-density.  It should lie in the
//    range [0,1].
//
//    double *Q, equal to 1-P.  If Q is an input
//    value, it should lie in the range [0,1].
//
//    double *F, the upper limit of integration
//    of the F-density.  If this is an input value, it should lie in the
//    range [0, +infinity).
//
//    double *DFN, the number of degrees of
//    freedom of the numerator sum of squares.  If this is an input value,
//    it should lie in the range: (0, +infinity).
//
//    double *DFD, the number of degrees of freedom
...
//    the F-density.
//
//    double *Q, equal to 1-P.  If Q is an output value,
...
//    lie in the range: [0, +infinity).
//
//    double *DFN, the number of degrees of freedom
...
//
//    double *PNONC, the noncentrality parameter
...
//
//    double *Q:  If P is to be computed, the
//    Q is set to 1 - P.
//
//    double *F, the upper limit of integration
...
//
//    double P, the cumulation from 0 to F of
//    the negative binomial distribution.  It
//    should lie in the range [0,1].
//
//    double Q, equal to 1-P.  If Q is an input
//    value, it should lie in the range [0,1].
//
//    double F, the upper limit of cumulation of
...
//    range [0,+infinity).
//
//    double S, the number of successes.
...
//    range [0,1].
//
//    double OMPR, the value of (1-PR).
//    It should lie in the range [0,1].
//
//  Output:
//
//    double P, the cumulation from 0 to F of
...
//
//    double F, the upper limit of cumulation of
//    the binomial distribution.  If it is an output value, it will be searched
//    for in the range [0,1.0D+300].
//
//    double S, the number of successes.
...
//    range [0,1].
//
//    double OMPR, the value of (1-PR).  It should lie in the range [0,1].
//
//    int STATUS, reports the status of the computation.
//     0, if the calculation completed correctly;
//    -I, if the input parameter number I is out of range;
...
//
//    double *P, the integral from -infinity to X
...
//    the Normal density.
//
//    double *MEAN, the mean of the Normal density.
//
//    double *SD, the standard deviation of the
...
//    Normal density.  
//
//    int *STATUS, the status of the calculation.
//    0, if calculation completed correctly;
//    -I, if input parameter number I is out of range;
...
//
//    double *P, the cumulation from 0 to S of the
...
//
//    double *P, the integral from -infinity to T of
...
//    value, it should lie in the range [0,1].
//
//    double *T, the upper limit of integration of
...
//    double *X, the argument of the function.
//
//    double *LAMBDA, the noncentrality parameter.
//
//    int *DF, the number of degrees of freedom.
//
//    double *CDF, the noncentral chi CDF.
//
{
# define N_MAX 27
...
//    output value of N_DATA will be 0 again.
//
//    int *A, the parameter of the function.
//
//    double *X, the argument of the function.
//
//    double *FX, the value of the function.
...
//  Input:
//
//    double *S, the upper limit of summation.
//
//    double *XN, the number of trials.
//
//    double *PR, the probability of success in one trial.
//
//    double *OMPR, equals ( 1 - PR ).
//
//  Output:
//
//    double *CUM, the cumulative binomial distribution.
//
//    double *CCUM, the complement of the cumulative
...
//  Output:
//
//    double *CUM, the cumulative chi-square distribution.
//
//    double *CCUM, the complement of the cumulative
//    chi-square distribution.
//
...
//    double *F, the upper limit of integration.
//
//    double *DFN, *DFD, the number of degrees of
//    freedom for the numerator and denominator.
//
//  Output:
//
//    double *CUM, *CCUM, the value of the F CDF and
...
//    double *F, the upper limit of integration.
//
//    double *DFN, *DFD, the number of degrees of freedom
...
//  Output:
//
//    double *CUM, *CCUM, the noncentral F CDF and
//    complementary CDF.
//
{
...
//    double *X, the upper limit of integration.
//
//    double *A, the shape parameter of the incomplete
//    Gamma distribution.
//
//  Output:
//
//    double *CUM, *CCUM, the incomplete Gamma CDF and
//    complementary CDF.
//
{
  int K1 = 0;
...
//  Input:
//
//    double *F, the number of failures.
//
//    double *S, the number of successes.
//
//    double *PR, *OMPR, the probability of success on
//    each binomial trial, and the value of (1-PR).
//
//  Output:
//
//    double *CUM, *CCUM, the negative binomial CDF,
...
//  Input:
//
//    double *ARG, the upper limit of integration.
//
//  Output:
//
//    double *CUM, *CCUM, the Normal density CDF and
//    complementary CDF.
//
//  Local:
//
...
//  Input:
//
//    double *S, the upper limit of cumulation of the
//    Poisson density function.
//
//    double *XLAM, the mean of the Poisson distribution.
//
//  Output:
//
//    double *CUM, *CCUM, the Poisson density CDF and
//    complementary CDF.
//
{
  double chi;
...
//  Input:
//
//    double *T, the upper limit of integration.
//
//    double *DF, the number of degrees of freedom of
//    the T distribution.
//
//  Output:
//
//    double *CUM, *CCUM, the T distribution CDF and
//    complementary CDF.
//
{
  double a;
...
//  Input:
//
//    double *P, *Q, the probability, and the complementary
//    probability.
//
//  Output:
//
//    double DINVNR, the argument X for which the
...
//    sets QLEFT and QHI.
//
//    double X, the value at which F(X) is to be evaluated.
//
//    bool QLEFT, is defined only if QMFINV returns FALSE.  In that
...
//
//    bool QHI, is defined only if QMFINV returns FALSE.  In that
...
//  Input:
//
//    double *X, the value at which the Normal CDF is to be
//    evaluated.  It is assumed that 5 <= abs ( X ).
//
//  Output:
//
//    double DLANOR, the logarithm of the asymptotic
//    Normal CDF.
//
{
...
//  Input:
//
//    double *Z, the value at which the Sterling
//    remainder is to be calculated.  Z must be positive.
//
//  Output:
//
//    double DSTREM, the Sterling remainder.
//
{
...
//  Input:
//
//    double *P, *Q, the value whose inverse from the
//    T distribution CDF is desired, and the value (1-P).
//
//    double *DF, the number of degrees of freedom of the
//    T distribution.
//
//  Output:
//
//    double DT1, the approximate value of X for which
...
//
//    logical QLEFT: .TRUE. if the stepping search terminated 
//    unsucessfully at XLO.  If it is .FALSE. the search terminated
//    unsucessfully at XHI.
//
//    logical QHI: .TRUE. if F(X) .GT. Y at the termination of the
...
//  Output:
//
//    double ERROR_F, the value of the error function at X.
//
{
...
//    output value of N_DATA will be 0 again.
//
//    int *A, int *B, the parameters of the function.
//
//    double *X, the argument of the function.
//
//    double *FX, the value of the function.
...
//  Input:
//
//    double mag: the magnitude to be signed.
//
//    double sign: a quantity whose sign is to be given to mag.
//
//  Output:
//
//    double mag: now has the same sign as sign.
//
{
  if (mag < 0) mag = -mag;
  if (sign < 0) mag = -mag;
...
//  Output:
//
//    long fifmod: the modulo of a and b.
//
{
  return ( a % b );
}
...
//  Input:
//
//    double *A, *B, parameters of the function.
//
//    double *X, the point at which the function is to
//    be evaluated.
//
//    double *EPS, a tolerance.
//
//  Output:
//
//    double FPSER, the value of IX(A,B)(X).
//
{
  double an;
  double c;
...
//  Input:
//
//    char *MSG, the message to be printed.
//
{
  printf ( "%s\n", msg );
...
//  Input:
//
//    double *A, forms the argument of the Gamma function.
//
//  Output:
//
//    double GAM1, the value of 1 / GAMMA ( A + 1 ) - 1.
//
{
...
//  Input:
//
//    double *A, *X, the arguments of the incomplete
//    gamma ratio.  A and X must be nonnegative.  A and X cannot
//    both be zero.
//
//    int *IND, indicates the accuracy request:
...
//  Output:
//
//    double *ANS, *QANS.  On normal output,
...
//  Input:
//
//    double *A, the argument of the function.
//    A should be positive.
//
//  Output:
//
//    double GAMMA_LOG, the value of ln ( Gamma ( A ) ).
//
{
...
//  Input:
//
//    double *A, *X, the parameters of the functions.
//    It is assumed that A <= 1.
//
//    double *R, the value exp(-X) * X^A / Gamma(A).
//
//    double *EPS, the tolerance.
//
//  Output:
//
//    double *P, *Q, the values of P(A,X) and Q(A,X).
//
{
  int K2 = 0;
  double a2n;
...
//  Input:
//
//    double *A, *B, values whose sum is the argument of
//    the Gamma function.
//
//  Output:
//
//    double GSUMLN, the value of ln(Gamma(A+B)).
//
{
...
//  Input:
//
//    int *I, the index of the desired constant.
//
//  Output:
//
//    int IPMPAR, the value of the desired constant.
//
{
  int imach[11];
  int ipmpar;
...
//  Input:
//
//    double *XX, the argument of the psi function.
//
//  Output:
//
//    double PSI, the value of the psi function.  PSI
...
//  Output:
//
//    double RLOG, the value of the function.
//
{
...
//  Output:
//
//    double RLOG1, the value of X - ln ( 1 + X ).
//
{
  double a = .566749439387324e-01;
  double b = .456512608815524e-01;
...
//  Input:
//
//    double *P, the probability whose normal deviate
//    is sought.
//
//  Output:
//
//    double STVALN, the normal deviate whose probability
//    is P.
//
{
  double xden[5] = {
