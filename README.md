...
scope. This is a machine-translated port of the FORTRAN90 code. Other
libraries, such as [`statrs`], can use the high-precision functions provided by
CDFLIB to build more ergonomic APIs.
...
SciPy's [incomplete-Γ/Β routines]. It delivers near-machine precision
...
working for large _a_ in the regularized incomplete Γ—exactly where
χ² tests with many degrees of freedom land. CDFLIB's Tricomi/Temme
...
The code has been tested against the original FORTRAN sources using three
different LLMs. In the process, we found [serious bugs in `rmathlib`] and a
quite serious bug in the C/C++ translation of CDFLIB (a coefficient for the
computation of the error function had been transcribed with the wrong exponent).
